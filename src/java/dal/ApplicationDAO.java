...
public class ApplicationDAO extends DBContext {
    ...
    public List<Application> getListAppOrderByDate() {
        List<Application> list = new ArrayList<>();
        String sql = "select top(4)*from Apps order by TotalRevenue desc;";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
    ...
    public List<Application> getListAppOrderBySalary() {
        List<Application> list = new ArrayList<>();
        String sql = "select top(4)*from Apps order by TotalRevenue desc;";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
    ...
    public List<Application> getListAppOrderByDown() {
        List<Application> list = new ArrayList<>();
        String sql = "select top(4)*from Apps where Price=0 order by DownloadCount desc";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
    ...
    public List<Application> getListByName(String id) {
        List<Application> list = new ArrayList<>();
        //lenh sql select * from categories cach 1:
        String sql = "select*from Apps where AppName like ?;";
        //cach 2: vao sql phai chuot vao bang chon scriptable as
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            st.setString(1, "%" + id + "%");
